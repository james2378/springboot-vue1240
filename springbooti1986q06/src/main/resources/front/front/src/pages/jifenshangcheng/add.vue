    ...
    >
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="商品详情" prop="shangpinxiangqing">
            <el-input v-model="ruleForm.shangpinxiangqing" 
                placeholder="商品详情" clearable :disabled=" false  ||ro.shangpinxiangqing"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="商品数量" prop="shangpinshuliang">
            <el-input v-model.number="ruleForm.shangpinshuliang" 
                placeholder="商品数量" clearable :disabled=" false  ||ro.shangpinshuliang"></el-input>
          </el-form-item>
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`jifenshangcheng/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('jifenshangcheng/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`jifenshangcheng/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
