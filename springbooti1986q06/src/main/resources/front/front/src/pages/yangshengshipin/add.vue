...
<div :style='{"width":"100%","padding":"20px 10%","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="视频标题" prop="shipinbiaoti">
            <el-input v-model="ruleForm.shipinbiaoti" 
                placeholder="视频标题" clearable :disabled=" false  ||ro.shipinbiaoti"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="视频封面" v-if="type!='cross' || (type=='cross' && !ro.shipinfengmian)" prop="shipinfengmian">
            <file-upload
            tip="点击上传视频封面"
            action="file/upload"
            :limit="3"
            ...
            <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' class="upload" v-else label="视频封面" prop="shipinfengmian">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="视频内容" v-if="type!='cross' || (type=='cross' && !ro.shipinneirong)" prop="shipinneirong">
            <file-upload
            tip="点击上传视频内容"
            action="file/upload"
            :limit="1"
		  ...
		  <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' class="upload" v-else label="视频内容" prop="shipinneirong">
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="视频简介" prop="shipinjianjie">
            <el-input
              type="textarea"
              :rows="8"
              placeholder="视频简介"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`yangshengshipin/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('yangshengshipin/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`yangshengshipin/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
