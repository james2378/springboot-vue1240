...
<div :style='{"width":"100%","padding":"20px 10%","margin":"10px auto","position":"relative","background":"#fff"}'>
    <el-form
      ...
      label-width="150px"
    >
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="预约体检" prop="yuyuetijian">
            <el-input v-model="ruleForm.yuyuetijian" 
                placeholder="预约体检" clearable :disabled=" false  ||ro.yuyuetijian"></el-input>
          </el-form-item>
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="体检结果" prop="tijianjieguo">
            <el-input v-model="ruleForm.tijianjieguo" 
                placeholder="体检结果" clearable :disabled=" false  ||ro.tijianjieguo"></el-input>
          </el-form-item>
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="体检报告" prop="tijianbaogao">
            <file-upload
            tip="点击上传体检报告"
            action="file/upload"
            :limit="1"
          ...
          <el-form-item :style='{"padding":"10px","margin":"0 0 10px"}' label="体检时间" prop="tijianshijian">
              <el-date-picker
				  :disabled=" false  ||ro.tijianshijian"
...
<script>
  export default {
    ...
    // 多级联动参数
      // 多级联动参数
      info() {
        this.$http.get(`tijianjiankangxinxi/detail/${this.$route.query.id}`, {emulateJSON: true}).then(res => {
          if (res.data.code == 0) {
			...
			this.$refs["ruleForm"].validate(valid => {
				if(valid) {
					...
					if(crossrefid && crossuserid) {
						...
						this.$http.get('tijianjiankangxinxi/list', {
							params: params
						}).then(res => {
							if(res.data.data.total>=crossoptnum) {
								...
								this.$http.post(`tijianjiankangxinxi/${this.ruleForm.id?'update':this.centerType?'save':'add'}`, this.ruleForm).then(res => {
									if (res.data.code == 0) {
										this.$message({
											message: '操作成功',
											type: 'success',
