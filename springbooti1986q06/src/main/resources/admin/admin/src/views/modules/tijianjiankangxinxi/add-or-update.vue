<template>
	<div class="addEdit-block" :style='{"padding":"30px"}'>
		...
		>
			<template >
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="预约体检" prop="yuyuetijian">
					<el-input v-model="ruleForm.yuyuetijian" placeholder="预约体检" clearable  :readonly="ro.yuyuetijian"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="预约体检" prop="yuyuetijian">
					<el-input v-model="ruleForm.yuyuetijian" placeholder="预约体检" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="预约时间" prop="yuyueshijian">
					<el-input v-model="ruleForm.yuyueshijian" placeholder="预约时间" clearable  :readonly="ro.yuyueshijian"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="预约时间" prop="yuyueshijian">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-if="type!='info'"  label="体检结果" prop="tijianjieguo">
					<el-input v-model="ruleForm.tijianjieguo" placeholder="体检结果" clearable  :readonly="ro.tijianjieguo"></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else class="input" label="体检结果" prop="tijianjieguo">
					<el-input v-model="ruleForm.tijianjieguo" placeholder="体检结果" readonly></el-input>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="upload" v-if="type!='info'&& !ro.tijianbaogao" label="体检报告" prop="tijianbaogao">
					<file-upload
						tip="点击上传体检报告"
						action="file/upload"
						:limit="1"
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.tijianbaogao" label="体检报告" prop="tijianbaogao">
					<el-button :style='{"border":"0","cursor":"pointer","padding":"0 15px","margin":"0 20px 0 0","outline":"none","color":"rgba(255, 255, 255, 1)","borderRadius":"4px","background":"#000","width":"auto","lineHeight":"40px","fontSize":"14px","height":"40px"}' type="text" size="small" @click="download($base.url+ruleForm.tijianbaogao)">下载</el-button>
				</el-form-item>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="!ruleForm.tijianbaogao" label="体检报告" prop="tijianbaogao">
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="date" v-if="type!='info'" label="体检时间" prop="tijianshijian">
					<el-date-picker
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="input" v-else-if="ruleForm.tijianshijian" label="体检时间" prop="tijianshijian">
					<el-input v-model="ruleForm.tijianshijian" placeholder="体检时间" readonly></el-input>
				</el-form-item>
			</template>
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' class="textarea" v-if="type!='info'" label="体检内容" prop="tijianneirong">
					<el-input
				...
				<el-form-item :style='{"width":"100%","margin":"0 0 20px 0"}' v-else-if="ruleForm.tijianneirong" label="体检内容" prop="tijianneirong">
					<span :style='{"fontSize":"14px","lineHeight":"40px","color":"#333","fontWeight":"500","display":"inline-block"}'>{{ruleForm.tijianneirong}}</span>
				</el-form-item>
...
export default {
	...
	methods: {
		...
		init(id,type) {
			...
			if(this.type=='info'||this.type=='else'){
				...
				for (var o in obj){
						if(o=='yuyuetijian'){
							this.ruleForm.yuyuetijian = obj[o];
							this.ro.yuyuetijian = true;
							continue;
						}
						if(o=='yuyueshijian'){
    ...
    info(id) {
      this.$http({
        url: `tijianjiankangxinxi/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `tijianjiankangxinxi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.tijianjiankangxinxiCrossAddOrUpdateFlag = false;
											this.parent.search();
